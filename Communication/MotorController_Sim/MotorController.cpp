...
MotorData MotorController::update()
{
    ...
    int pwr = (int)fabs(pwm_prev);

    float Vin = (pwm_prev / 255.0) * V_supply;
    sim_i += deltaT * (Vin - R * sim_i - Ke * sim_omega) / L;
    sim_omega += deltaT * (Kt * sim_i - b * sim_omega - external_load) / J;

    if (fabs(sim_omega) < 0.01 && fabs(sim_i * Kt) < external_load) {
        sim_omega = 0;
    }
...
void MotorController::setExternalLoad(float load) {
    external_load = load;
}