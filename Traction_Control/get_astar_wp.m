function path = get_astar_wp(start, goal, terrain, params)
    % Cost weights
    ...
    plot3(xs(path(:,1)),ys(path(:,2)), ...
          arrayfun(@(k) Z_grid(path(k,2),path(k,1)),1:size(path,1))' + 0.05,...
          'r','LineWidth',2);
    shading interp 
    colormap(gray) 
    axis equal
